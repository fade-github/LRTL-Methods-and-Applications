function [M9,CZ_l1] = update_Framelet_M(data,known,Nway,X,Y, M0,opts) % 这个优化函数求解很重要
    ...
    [D,R]=GenerateFrameletFilter(frame); % 这个函数表示什么
    nD=length(D);
    X_way=[n1,n2,Nway(end)];
    X_transition=Unfold(CastKet2Image22(M0,n1,n2,I1,J1),X_way,1);

    Z=FraDecMultiLevel(X_transition,D,Level); % 这个函数表示什么
