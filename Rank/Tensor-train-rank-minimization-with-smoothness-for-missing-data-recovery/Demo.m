...
X0 = double(imread('baboon.bmp')); % 数据读取
name = {'baboon'};
             

for SR = 0.1 % 设置随机缺失的比例
    
%% Sampling，这一步非常重要，给定算法输入参数形式   
sizeData = size(X0);

Y = zeros(sizeData);
Mask  = zeros(sizeData);
Index = find(rand(prod(sizeData),1)<SR); % 产生随机缺失的机制
Y(Index) = X0(Index); % 满足初始条件
Mask(Index) = 1; 

Otrue  = CastImageAsKet22( X0, Nway, I1 ,J1 ); % 在TT_framelet文件夹中，考虑修改该函数处理张量的维度
...
imname=[num2str(name{1}),'_miss_SR_',num2str(SR),'.mat']; % 保存需要补全的张量
...
if EN_HaLRTC
    ...
    for beta = 5*1e-5
        opts.beta = beta;
        tic;
        X = LRTC_HaLRTC( Y, Index, opts ); % 参数Y是初始化的补全张量，注意修改opts超参数列表
        time=toc;
...
if EN_tSVD
    ...
    for k = 1:length(beta1)
        ...
        fprintf('psnr=%.2f,ssim=%.4f,beta=%.6f\n', psnr, ssim, beta)
        fprintf('==================================\n')
...
if EN_SiLRTCTT
    ...
    for k = 1:length(r)
        ...
        tic;
        [X, Out] = SiLRTC_TT( Okn, Oknown, Nway, opts ); % minimizes the convex nuclear norm surrogate of TT rank

        X = CastKet2Image22(X,256,256,I1,J1); % 与算法中的Nway相对应
        time=toc;
        psnr =  psnr3(X0/255,X/255);        
        Ssim=zeros(1,3);
        for i=1:1:3
...
if EN_TMacTT
    ...
    opts.th = th;
        ...
        tic;
        [X, Out] = TMac_TT( Okn, Oknown, Nway, opts ); % factorizes each unfolding matrix M into the product of two smaller matrices
...
%% use TT-Framelet (Tensor train rank under framelet and total variation regularization), global and local smoothness
j = j+1;
if EN_TT_Framelet
    opts=[];
    ...
    disp(['performing ',methodname{j}, ' ... ']);
        ...
        tic;
        % 返回补全好的张量以及算法运行时间
        [X_TT_Framelet, Out_TT_Framelet] = TT_Framelet( Okn, Oknown, Nway, opts ); % 本文所提出来的算法(Tensor Train Under Penalty)，用framelet and total variation regularization来限制局部秩大小
        ...
        save(imname,'X_TT_Framelet','time'); % 最终输出结果，是对张量Y补全之后的结果，与初始张量X0进行比较      
end  
end