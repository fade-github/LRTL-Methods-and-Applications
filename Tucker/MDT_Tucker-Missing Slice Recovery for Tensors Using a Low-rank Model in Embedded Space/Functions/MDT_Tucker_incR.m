...
function [Xest, histo, histoR, G, U, S, D] = MDT_Tucker_incR(T,Q,tau,param)
  ...
  if length(tau) == 1 % 控制MDT参数
    tau = [tau 1];
    flag = 1; % True，condition has been met
  else
    flag = 0;
  end
  
  [v0, S] = DE_tensor_all(ones(II),tau,[]); % Self-defined function，Delay Embedded 数据预处理，此时的张量v0只是初始化
  II2 = size(v0);
  if mod(length(II2),2)
    II2 = [II2 1];
  end
  ...
  D = DE_tensor_all_adjoint(v0,S); % 返回张量，用于之后的 inverse MDT

  % Step1 : Multi-way delay embedding transform (MDT)
  Tms = zeros(II);
  Tms(Q(:)==1)= T(Q(:)==1);
  QQ = DE_tensor_all(Q,tau,S); % MDT张量转化，mask tensor Q
  V = DE_tensor_all(Tms,tau,S); % MDT张量转化，missing tensor 

  % Step2 : Tucker decomposition & its initialization
  if isempty(incR)
    for n = 1:Ni
      incR{n} = [1:II2(n)]; % 给定递增秩的列表
    end
  end
  for n = 1:Ni
    R(n) = incR{n}(1);
    U{n} = orth(randn(II2(n),R(n))); % 正交的factor矩阵限制，初始化Tucker分解
  end
  G = randn(R); % 初始化Tucker分解

  [Vest, G, U, histo, histoR] = Tucker_completion_incR(V,QQ,G,U,delta,incR,maxiter,inloop,tol,verb);

  % Step3 : inverse MDT
  Xest = DE_tensor_all_adjoint(Vest,S) ./ D; % 返回补全的数据
