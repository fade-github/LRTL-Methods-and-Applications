...
function [X, G, U, histo, histoR] = Tucker_completion_incR(T,Q,G,U,delta,incR,maxiter,inloop,tol,verb)
  ...
  Ri = ones(1,N);
  for n = 1:N
    R(n) = incR{n}(1); % 下标1说明cell数组每个元素只有1个数值，此时给定矩阵秩大小，替换之前factor矩阵的R取值
  end
  for iter = 1:maxiter
    ...
    % calc. mode residual,本算法的创新之处，按照残差最大的方向增加秩的大小
      dX = Q.*(T-GU); % 张量残差
      for n = 1:N
        E{n} = tensor_allprod_exc(dX,U,1,n,II); % 任意去除n-mode下的张量残差乘积
        mode_residual(n) = norm(E{n}(:));
      end
      
      [~, idd] = max(mode_residual);
      Ri(idd)= min(length(incR{idd}),Ri(idd) + 1);
      R(idd) = min(II(idd),incR{idd}(Ri(idd))); % 指定哪个维度上factor矩阵的秩进行increasing
